...
3. Подключить Postgres и SQLAlchemy (async).
4. Настроить Alembic (миграции).
5. Написать Dockerfile для backend и docker-compose.
...
## 3. ЛОББИ И СТОЛЫ (REST)

### Tables
13. Создать модель Table (стол).
14. Создать модель Seat (место игрока).
15. Эндпоинт `GET /tables` — список столов.
16. Эндпоинт `POST /tables/create` — создание стола.
17. Эндпоинт `GET /tables/{id}` — инфо стола.
18. Эндпоинт `POST /tables/{id}/join` — сесть за стол.
19. Эндпоинт `POST /tables/{id}/leave` — покинуть стол.
20. Эндпоинт `POST /tables/{id}/spectate` — наблюдение.
21. Тесты для лобби/столов.

---

## 4. ПОКЕРНЫЙ ДВИЖОК (ENGINE)

### Core Engine
22. Создать класс Card + enum Rank/Suit.
23. Создать класс Deck (shuffle, draw, draw_many).
24. Создать класс PlayerState (stack, карты, статус).
25. Создать класс GameState (фаза, банк, борд, игроки).
26. Реализовать переходы фаз (preflop → river → showdown).
27. Реализовать действия fold/check/call/bet/raise.
28. Реализовать HandEvaluator (оценка комбинаций).
29. Реализовать определение победителей.
30. Написать юнит-тесты на комбинации.
31. Написать юнит-тесты на игровой сценарий.

---

## 5. GAME SERVICE (СЕРВИСНАЯ ЛОГИКА ИГРЫ)
...
### Stats
50. Создать модель PlayerStats.
51. Реализовать функцию обновления статы после раздачи.
...
### Frontend
57. Инициализировать проект React + Vite + TS.
